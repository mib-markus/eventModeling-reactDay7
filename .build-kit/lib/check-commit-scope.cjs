...
//     run(ctx) { return [{ path: 'some/file.tsx', reason: 'why' }]; },
//   };
//
// This is the UI stack's copy. It differs from a backend stack's in one way that
// matters everywhere: a slice folder here is THREE segments deep,
// src/slices/{context}/{lane}/{slice}/, because a screen's actor lane becomes a
// path segment (see .build-kit/CLAUDE.md guideline 8). A two-segment pattern
// matches nothing in this repo, so the guard would silently pass every commit.
//
// Zero dependencies — plain Node, so it works from git's pre-commit hook
// (../../.githooks/pre-commit), from `npm run run:checks`, or from CI.
...
// top-level — required when this project is a subdirectory of a larger repo:
// without it every path comes back prefixed, SLICE_PATTERN never matches, and
// the whole guard silently no-ops. `git ls-files` is already cwd-relative.
...
function main() {
  ...
  const ctx = {
    changes,
    touchesSlice,
    // Deliberately process.cwd(), not `git rev-parse --show-toplevel` — see the
    // --relative note above; every path here is relative to this project's own
    // root, not an outer monorepo's.
